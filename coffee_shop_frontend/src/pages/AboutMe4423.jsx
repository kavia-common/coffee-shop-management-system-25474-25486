 ...
 * It loads page-specific CSS and a small JS helper safely, scoped to #about-me-442-3-root.
 * No HTML files are fetched; images are referenced from /assets/figmaimages/..., prefixed with PUBLIC_URL for React.
 */
function AboutMe4423() {
  const base = process.env.PUBLIC_URL || '';

  // Build the static markup from the HTML inner content only, with normalized image paths.
  const content = useMemo(() => {
    const html = `
      <main id="screen-442-3" aria-labelledby="headline-title" role="main">
        ...
        <nav class="frame frame-studio-name" aria-label="Breadcrumb">
          <div class="group text-breadcrumb" role="group" aria-label="Breadcrumb path">
            ...
            <div class="icon-arrow" role="img" aria-label="arrow right">
              <img src="/assets/figmaimages/figma_image_442_32.png" alt="Arrow right" />
            </div>
        ...
        <figure class="frame frame-image" role="img" aria-label="Saly 39 illustration">
          <img src="/assets/figmaimages/figma_image_442_9.png" alt="Saly 39 illustration" />
        </figure>
    ...
    `;
    // Prefix with PUBLIC_URL for React when injecting content
  ...
  return <div id="about-me-442-3-root" dangerouslySetInnerHTML={{ __html: content }} />;
}
